...
$stmt = $pdo->prepare("INSERT INTO aoi_hp_login(uname,uid,upass,indate)VALUES(:uname,:uid,:upass,sysdate());");
...
$stmt->bindValue(':upass', h($upass), PDO::PARAM_INT);  //Integer（数値の場合 PDO::PARAM_INT)
$status = $stmt->execute();
...
if($status==false){
  ...
  header('Location: login.php');//Location:の後ろの半角スペースは必ず入れる。
  exit();
